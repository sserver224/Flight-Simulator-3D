...
from XInput import *
hg.LoadPlugins()
def get_resouce_path(relative_path):
        try:
...
class WaterReflection():
        ...
        def load_parameters(self, file_name=get_resouce_path("assets/scripts/water_reflection.json")):
        ...
        def save_parameters(self, output_filename=get_resource_path("assets/scripts/water_reflection.json")):
...
class SeaRender:
        def __init__(self, plus, scene, render_script: hg.RenderScript = None):
                ...
                self.shader = renderer.LoadShader(get_resource_path("assets/shaders/sky_sea_render_optim.isl"))

                self.noise_texture_1 = renderer.LoadTexture(get_resource_path("assets/textures/noise.png"))
                self.noise_texture_2 = renderer.LoadTexture(get_resource_path("assets/textures/noise_3.png"))
                self.noise_displacement_texture = renderer.LoadTexture(get_resource_path("assets/textures/noise_2.png"))
                self.stream_texture = renderer.LoadTexture(get_resource_path("assets/textures/stream.png"))
                self.clouds_map = renderer.LoadTexture(get_resource_path("assets/textures/clouds_map.png"))

                self.clouds_scale = hg.Vector3(1000., 0.1, 1000.)
                self.clouds_altitude = 1000.
                self.clouds_absorption = 0.1

                self.tex_sky_N = renderer.LoadTexture(get_resource_path("assets/skymaps/clouds.png"))
        ...
        def load_json_script(self, file_name=get_resource_path("assets/scripts/sea_parameters.json")):
                json_script = hg.GetFilesystem().FileToString(file_name)
                if json_script != "":
                        script_parameters = json.loads(json_script)
        ...
        def save_json_script(self, output_filename=get_resource_path("assets/scripts/sea_parameters.json")):
...
class Missile(Destroyable_Machine):
        ...
        def __init__(self, name, nationality, plus, scene, audio, missile_file_name, smoke_file_name_prefix,
                ...
                # Feed-backs:
                self.explode = ParticlesEngine(self.name + ".explode", plus, scene, get_resource_path("assets/feed_backs/feed_back_explode.geo"),
        ...
        def start(self, target: Destroyable_Machine, v0: hg.Vector3):
                if not self.activated:
                        ...
                        self.turbine_channel = self.audio.Start(self.audio.LoadSound(get_resource_path("assets/sfx/missile_engine.wav")),
        ...
        def start_explosion(self):
                if not self.wreck:
                        ...
                        self.audio.Start(self.audio.LoadSound(get_resource_path("assets/sfx/missile_explosion.wav")), self.explosion_settings)
                        self.wreck = True
...
class MachineGun(ParticlesEngine):
        def __init__(self, name, plus, scene):
                ParticlesEngine.__init__(self, name, plus, scene, get_resource_path("assets/weaponry/gun_bullet.geo"), 24, hg.Vector3(2, 2, 20),
                ...
                for i in range(self.num_particles):
                        fb = ParticlesEngine(self.name + ".fb." + str(i), plus, scene, get_resource_path("assets/feed_backs/bullet_impact.geo"), 5,
...
class Aircraft(Destroyable_Machine):
        ...
        def __init__(self, name, nationality, id_string, plus, scene, start_position: hg.Vector3,
                ...
                self.smoke = ParticlesEngine(self.name + ".smoke", plus, scene, get_resource_path("assets/feed_backs/feed_back_explode.geo"), 400,
        ...
        def create_post_combustion_particles(self, plus, scene, engine_name):
                pc = ParticlesEngine(self.name + engine_name, plus, scene, get_resource_path("assets/feed_backs/bullet_impact.geo"), 15,
...
class AircraftSFX:
        ...
        def start_engine(self, main):
                ...
                self.air_chan = main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/air.wav")), self.air_settings)
                self.turbine_chan = main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/turbine.wav")), self.turbine_settings)
                self.pc_chan = main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/post_combustion.wav")), self.pc_settings)
                self.start = True
                self.pc_started = False
        ...
        def stop_engine(self, main):
                ...
                self.wind_chan = main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/wind.wav")), self.wind_settings)
        ...
        def update_sfx(self, main, dts):
                ...
                if self.aircraft.wreck and not self.exploded:
                        ...
                        main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/explosion.wav")), self.explosion_settings)
                        self.exploded = True
                ...
                if self.aircraft.gun_machine.num_new > 0:
                        ...
                        self.wind_chan = main.audio.Start(main.audio.LoadSound(get_resource_path("assets/sfx/machine_gun.wav")), self.machine_gun_settings)
...
class Clouds:
        ...
        def load_json_script(self, file_name=get_resource_path("assets/scripts/clouds_parameters.json")):
                json_script = hg.GetFilesystem().FileToString(file_name)
                if json_script != "":
                        script_parameters = json.loads(json_script)
                        self.__setstate__(script_parameters)

        def save_json_script(self, scene, output_filename=get_resource_path("assets/scripts/clouds_parameters.json")):
...
def update_radar(Main,plus,aircraft,targets):
        ...
        plus.Sprite2D(rx,ry,rs,get_resource_path("assets/sprites/radar.png"),hg.Color(1,1,1,value))
        ...
        plus.Sprite2D(rx, ry, rs, get_resource_path("assets/sprites/radar_light.png"),hg.Color(1,1,1,0.3*value))
        plus.Sprite2D(rx, ry, rs, get_resource_path("assets/sprites/radar_box.png"),c)
...
def update_machine_gun_sight(Main,plus,aircraft:Aircraft):
        ...
        if p2D is not None:
                plus.Sprite2D(p2D.x*Main.resolution.x, p2D.y*Main.resolution.y, 64/1600*Main.resolution.x, get_resource_path("assets/sprites/machine_gun_sight.png"), hg.Color(0.5,1,0.5, Main.HSL_postProcess.GetL()))
        ...
        if target is not None:
                ...
                if p2D is not None:
                        ...
                        plus.Sprite2D(p2D.x * Main.resolution.x, p2D.y * Main.resolution.y, 32 / 1600 * Main.resolution.x,
                                                  get_resource_path("assets/sprites/target_sight.png"), c)
                        ...
                        if aircraft.target_locking_state>0:
                                ...
                                plus.Sprite2D(p2D.x * Main.resolution.x, p2D.y * Main.resolution.y, 32 / 1600 * Main.resolution.x,
                                                          get_resource_path("assets/sprites/missile_sight.png"), c)
...
class Main:
        ...
        title_font = get_resource_path("assets/fonts/destroy.ttf")
        hud_font = get_resource_path("assets/fonts/Furore.otf")
...
def init_game(plus):
        ...
        for i in range(4):
                Main.p1_missiles[i] = Missile("sidewinder_" + str(i), 1, plus, Main.scene, Main.audio,
                                                                          get_resource_path("assets/weaponry/enemymissile_low.geo"), get_resource_path("assets/weaponry/enemymissile_smoke"))
        ...
        Main.texture_hud_plot_aircraft = plus.LoadTexture(get_resource_path("assets/sprites/plot.png"))
        Main.texture_hud_plot_missile = plus.LoadTexture(get_resource_path("assets/sprites/plot_missile.png"))
        Main.texture_hud_plot_ship = plus.LoadTexture(get_resource_path("assets/sprites/plot_ship.png"))
        Main.texture_hud_plot_dir = plus.LoadTexture(get_resource_path("assets/sprites/plot_dir.png"))
        Main.texture_noise = plus.LoadTexture(get_resource_path("assets/sprites/noise.png"))
...
def load_scene_parameters(file_name=get_resource_path("assets/scripts/scene_parameters.json")):
        json_script = hg.GetFilesystem().FileToString(file_name)
...
def save_scene_parameters(output_filename=get_resource_path("assets/scripts/scene_parameters.json")):
        environment = Main.scene.GetEnvironment()
...
def load_game_parameters(file_name=get_resource_path("assets/scripts/dogfight.json")):
        json_script = hg.GetFilesystem().FileToString(file_name)
        if json_script != "":
                script_parameters = json.loads(json_script)
...
def save_game_parameters(output_filename=get_resource_path("assets/scripts/dogfight.json")):
...
def post_processes_save_parameters(output_filename=get_resource_path("assets/scripts/post_render.json")):
...
def post_processes_load_parameters(file_name=get_resource_path("assets/scripts/post_render.json")):
        json_script = hg.GetFilesystem().FileToString(file_name)
        if json_script != "":
                script_parameters = json.loads(json_script)
...
def load_fps_matrix(fps):
        pos, rot = load_json_matrix(get_resource_path("assets/scripts/camera_position.json"))
...
def init_scene(plus):
        ...
        plus.LoadScene(Main.scene, get_resource_path("assets/aircraft/aircraft.scn"))
        plus.LoadScene(Main.scene, get_resource_path("assets/ennemyaircraft/ennemy_aircraft.scn"))
        plus.LoadScene(Main.scene, get_resource_path("assets/aircraft_carrier/aircraft_carrier.scn"))
        plus.LoadScene(Main.scene, get_resource_path("assets/island/island.scn"))
        plus.LoadScene(Main.scene, get_resource_path("assets/feed_backs/feed_backs.scn"))
        ...
        # ---- Clouds:
        json_script = hg.GetFilesystem().FileToString(get_resource_path("assets/scripts/clouds_parameters.json"))
        if json_script != "":
        ...
        Main.sea_render_script = hg.RenderScript(get_resource_path("assets/lua_scripts/sea_render.lua"))
        ...
        if hg.ImGuiBegin("Scene Settings"):
                ...
                if hg.ImGuiButton("Save camera"):
                        save_json_matrix(camera.GetTransform().GetPosition(), camera.GetTransform().GetRotation(),
                                                         get_resource_path("assets/scripts/camera_position.json"))

                if hg.ImGuiButton("Load aircraft matrix"):
                        pos, rot = load_json_matrix(get_resource_path("assets/scripts/aircraft_position.json"))
                ...
                if hg.ImGuiButton("Save aircraft matrix"):
                        ...
                        save_json_matrix(nd.GetTransform().GetPosition(), nd.GetTransform().GetRotation(),
                                                         get_resource_path("assets/scripts/aircraft_position.json"))
...
def control_aircraft_paddle(dts, aircraft: Aircraft):
        ...
        aircraft.set_thrust_level(max(0, v))
        aircraft.set_brake_level(max(0, -1*v))
...
def start_phase(plus, delta_t):
        ...
        if Main.fading_cptr >= fade_in_delay:
                ...
                plus.Text2D(x, y / 900 * Main.resolution.y, "Left Stick Up", s * Main.resolution.y, c, Main.hud_font)
                plus.Text2D(x, (y - 20) / 900 * Main.resolution.y, "Right Stick", s * Main.resolution.y, c, Main.hud_font)
                plus.Text2D(x, (y - 40) / 900 * Main.resolution.y, "Right Stick", s * Main.resolution.y, c, Main.hud_font)
                plus.Text2D(x, (y - 60) / 900 * Main.resolution.y, "Left stick Left/Right", s * Main.resolution.y, c, Main.hud_font)
                ...
                plus.Text2D(x, (y - 140) / 900 * Main.resolution.y, "Left Stick Down", s * Main.resolution.y, c, Main.hud_font)
