...
class AircraftUserControlDevice(ControlDevice):
    ...
    def update(self, dts):
        if self.activated:
            if get_connected()[0]:
                self.machine.set_yaw_level(get_thumb_values(get_state(0))[1][0])
                self.machine.set_thrust_level(get_trigger_values(get_state(0))[1])
                self.machine.set_brake_level(get_trigger_values(get_state(0))[0])
                self.machine.set_roll_level(-1*get_thumb_values(get_state(0))[0][0])
                self.machine.set_pitch_level(get_thumb_values(get_state(0))[0][1])
...
def update_menu_phase(dts):
    ...
    if Main.fading_cptr >= fade_in_delay:
        ...
        if Main.flag_paddle:
            ...
            Overlays.add_text2D("RT",  hg.Vec2(x, (y) / 900), s, c, Main.hud_font)
            Overlays.add_text2D("Left stick vertical", hg.Vec2(x, (y - 20) / 900), s, c, Main.hud_font)
            Overlays.add_text2D("Left stick horizontal", hg.Vec2(x, (y - 40) / 900), s, c, Main.hud_font)
            Overlays.add_text2D("Right stick horizontal", hg.Vec2(x, (y - 60) / 900), s, c, Main.hud_font)
            ...
            Overlays.add_text2D("LT", hg.Vec2(x, (y - 140) / 900), s, c, Main.hud_font)
